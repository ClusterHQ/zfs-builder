...
function build {
    ...
    UNAME_R=$2
    LOCALVERSION=$3
    docker build --build-arg KERNEL_VERSION=$KERNEL --build-arg LOCALVERSION="-$LOCALVERSION" -t clusterhq/build-zfs-boot2docker:${UNAME_R} -f Dockerfile.boot2docker .
...
# travis trusty XXX '30-generic' is a guess
build 3.19 3.19.0-30-generic 30-generic

# travis precise, probably an ubuntu kernel
#build 3.13 3.13.0-63-generic # XXX fails
